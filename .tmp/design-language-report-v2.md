...
> 基于对 21 个页面文件的深度分析 synthesized from three analytical passes across core pages, topic pages, and content pages.
...
--text-dark: #2c3e50;            /* login, task-monitor, landing */
```
...
| 功能页 (landing) | 12px–16px | feature-card 12px, intro 16px |
...
/* 登录卡片（带主题色） */
box-shadow: 0 4px 16px rgba(107, 144, 128, 0.12);  /* login, landing */
...
.btn {
  ...
  color: #fff;
  padding: 14px 32px;
  border-radius: 30px;                /* pill shape - 偏离站点标准 */
...
.feature-card {
  ...
  box-shadow: 0 2px 8px rgba(107, 144, 128, 0.06);
  transition: transform 0.2s, box-shadow 0.2s;
}
.feature-card:hover {
  transform: translateY(-4px);
  box-shadow: 0 6px 16px rgba(107, 144, 128, 0.12);
}
```
...
以下设计元素在整个站点的 21 个页面中表现出高度一致性，已形成事实上的设计公约：
...
2. **Noto Serif SC 的中文排版霸权**
   ...
   - 一致性：21 个文件中 17 个使用 Noto Serif SC（或其变体 Songti / SimSun）作为中文主字体，仅 login, task-monitor, wei-bei-guide（使用 Ma Shan Zheng + 系统字体）、landing（纯系统字体）例外。
...
5. **hover 上浮动效**
   - 证据：landing-v2 featured-card (`translateY(-4px)`), magazine-issue (`translateY(-6px)`), the-art-of-waiting (`translateY(-8px)`), cosmos-science (`translateY(-10px)`), 魏碑手册 (`translateY(-5px)`), landing (`translateY(-4px)`)。
   - 一致性：21 个文件中 19 个在卡片 hover 时使用 `translateY` 负方向位移，配合阴影加深。仅 login 和 task-monitor 因功能属性未采用此模式。

6. **左侧边框强调块**
   - 证据：magazine 的 featured highlight (3px copper), 书法史的 blockquote (4px red), 婆媳关系的章节卡片 (5px gradient left), Python 指南的 blockquote (left blue border), index-v3 的 story cards (3px jade), landing 的 feature-card (4px sage green)。
...
8. **粘性/固定导航**
   ...
   - 一致性：21 个文件中 18 个采用某种形式的固定或粘性导航，确保长文阅读时的位置感。
...
1. **CSS 变量命名体系的分裂**
   - 证据：landing-v2 使用 `--bg-primary` / `--text-primary` 前缀体系；index-v3 使用 `--color-ink` / `--color-cream` 前缀体系；书法史/书画鉴赏/宇宙哲学使用 `--accent-red` / `--gold-warm` 描述性命名；cosmos-science-v2 使用 `--color-cosmos` / `--color-starlight` 主题命名；task-monitor 使用 `--primary` / `--bg-page` 功能命名；landing 完全没有 CSS 变量，全部硬编码。
   - 结果：同一色值（如 #0d1f1c）在不同文件中有 4–5 种不同的变量名，部分页面（landing）完全没有变量系统。

2. **圆角尺度不统一**
   - 证据：magazine 的 read-btn 为 `0px`（纯矩形），内容页卡片多为 `8px`，poxi-analysis 的章节卡片为 `16px`，login 的登录卡片为 `16px`，cosmos-science 的卡片为 `16px` (1rem)，quality-sources 为 `12px` (rounded-xl)，landing 的 intro 为 `16px`、feature-card 为 `12px`、按钮为 `30px` (pill)。
...
3. **按钮样式的主题碎片化**
   - 证据：
     ...
     - landing：鼠尾草绿（`#6b9080`）+ 30px pill 圆角
   - 结果：同一功能的"主按钮"在 5 个集群中有 6 种截然不同的视觉处理。

4. **字体的英文展示字不统一**
   - 证据：magazine 使用 Playfair Display；宇宙哲学和 the-art-of-waiting 使用 Cormorant Garamond；cosmos-science 使用 Sora；quality-sources 使用 Inter；yehuimei-album 未使用专用英文展示字；landing 使用纯系统无衬线字体。
   - 结果：英文标题的气质在"经典权威"、"优雅复古"、"科技未来"、"现代简洁"之间摇摆。

5. **布局容器宽度标准不一**
   - 证据：叙事/散文页使用 800px（宇宙哲学、yehuimei-album）；参考/指南页使用 900px（书法史、书画鉴赏、Python、婆媳关系、landing）；dashboard 使用 1600px（task-monitor）；landing 使用 1200px（magazine、cosmos-science）。
   - 结果：长文阅读体验在 800px 和 900px 之间微妙切换，虽不明显但缺乏工程规范。

6. **阴影值的不一致**
   - 证据：landing-v2 使用极柔和的 `rgba(0,0,0,0.04)`；magazine 使用 `rgba(0,0,0,0.1)`；cosmos-science 使用硬黑 + 发光混合阴影；婆媳关系使用 `rgba(230, 126, 34, 0.15)`（带主题色）；魏碑手册使用 `rgba(139, 69, 19, 0.3)`（带主题色）；landing 使用 `rgba(107, 144, 128, 0.08)`（带鼠尾草绿色）。
   - 结果：阴影不仅是深度提示，还承担了"主题染色"功能，导致无法提取统一的 `--shadow-*` 变量。

7. **段落首行缩进 (text-indent: 2em) 的取舍不一致**
   - 证据：书法史、书画鉴赏、宇宙哲学、yehuimei-album、olympiad-math-philosophy 明确使用 `text-indent: 2em`；wei-bei-guide（Tailwind 默认）、Python 指南（部分弱化）、task-monitor / quality-sources / login / landing 完全不使用。
   - 结果：文学页有传统中文排版缩进，工具页没有，跨页阅读时版式感受断裂。

8. **动画/交互密度差异巨大**
   - 证据：cosmos-science 有 letter-slide-in 动画和 glow pulse；宇宙哲学有 starfield canvas 动画；landing-v2 有 quote gate fade-in；而 login、task-monitor、quality-sources、landing 几乎没有任何动画。
   - 结果：从宇宙科学页跳到 task-monitor 或 landing，页面仿佛从"电影"切换到了"Excel"。

9. **Tailwind 与手写 CSS 的框架分裂**
   - 证据：cosmos-science、quality-sources、wei-bei-guide 使用 Tailwind CSS CDN；其余 18 个文件使用手写 `<style>` 块或内联 CSS。
   - 结果：HTML 结构、类名命名、间距系统完全无法互通，维护成本极高。

10. **功能性页面的美学孤立（新增 landing）**
    - 证据：login.html 和 task-monitor.html 使用完全不同的配色（`#6b9080` 主色，无铜橙/金色，无衬线字体，无深色区域）；landing.html 同样使用 `#6b9080` 作为主色，采用系统无衬线字体而非 Noto Serif SC，没有使用站点的金-铜-橙色系，按钮使用 30px pill 圆角偏离站点标准。
    - 结果：这三个页面（login, task-monitor, landing）与站点的"数字书房"整体氛围完全脱节，像是被嵌入的第三方应用。landing.html 的配色方案与 login/task-monitor 完全一致，形成了"功能页美学集群"，与主站博雅风格形成鲜明对比。
...
- 手工页面（landing-v2、index-v3、magazine 系列、login、task-monitor、landing）则体现了不同创作阶段或不同负责人的设计决策，没有共享统一的 design tokens。
...
**D. 功能页与品牌页的脱节**
- login、task-monitor 和 landing 的设计目标是"完成登录"、"监控系统"和"站点入口"，其设计出发点纯粹是功能可用性，没有接受站点的品牌设计语言输入。这在小型项目中常见：功能页由后端或工具链模板直接生成，前端品牌规范未覆盖。landing.html 的加入使这一问题更加明显，三个页面形成了统一的"功能页孤岛"。
...
4. **为功能性页面注入品牌色（login / task-monitor / landing）**
   - login 和 landing 的主色应从 `#6b9080` 改为站点主 accent 色（铜橙/金色体系），并引入 Noto Serif SC 作为标题字体。
   - task-monitor 应至少将面板 header、关键状态色、页脚 seal 调整为与站点一致的设计语言，而非纯 dashboard 风格。
   - landing.html 作为站点入口，其品牌一致性影响最大，应优先整改。

5. **收敛圆角标准到 3 个值**
   - 规定：`0px` 仅用于 magazine read-btn 等极端编辑组件；`4px` 用于按钮、标签、输入框；`8px` 或 `12px` 用于卡片和容器。全面排查并替换页面中的 `16px` 和 `1rem` 圆角异常值（除非有特殊 UX 理由）。landing 的 30px pill 按钮应改为 `4px` 或 `8px`。
...
- [ ] 将 `design-tokens.css` 链接到所有 21 个 HTML 文件（可先以 `<link>` 形式引入，不立即替换旧变量）。
...
- [ ] 重新设计 `landing.html`：将主色调从 `#6b9080` 调整为铜橙/金色体系，引入 Noto Serif SC，按钮改为 `4px` 或 `8px` 圆角，加入 `.wn-seal` 页脚元素。
...
## Appendix: landing.html Analysis

### 文件信息
- **文件名**: `landing.html`
- **路径**: `/whitenote/docs/public/landing.html`
- **标题**: 浩哥的文字花园 - WhiteNote
- **页面定位**: 站点入口/着陆页，区别于 `landing-v2.html` 的编辑花园风格

### 色彩分析

`landing.html` 采用了一套与主站博雅风格明显偏离的配色体系：

| 元素 | 色值 | 说明 |
|------|------|------|
| 页面背景 | `#f7f8f9` | 冷调浅灰，偏离主站常见的米白/奶油暖调 |
| 主标题/强调色 | `#6b9080` | 鼠尾草绿，与 login/task-monitor 完全一致 |
| 卡片背景 | `#fff` | 纯白，而非主站惯用的 cream/parchment |
| 正文文字 | `#2c3e50` | 深蓝灰，接近功能页风格 |
| 次要文字 | `#666` / `#555` | 无统一变量管理 |
| 边框/分隔线 | `rgba(107, 144, 128, 0.15)` | 鼠尾草绿透明色，与主站的铜橙/金色强调无关 |
| 按钮 hover | `#5a7c6b` | 鼠尾草绿的深色调 |

值得注意的是，页面完全没有使用主站标志性的铜橙 `#e07b39`、古金 `#C9A961`、朱砂 `#B85450` 或深林墨绿 `#0d1f1c`。
...
- **中文主字体**: 纯系统无衬线，未使用 Noto Serif SC 或任何衬线字体
- **标题字体**: `h1` 使用系统 sans-serif，`font-weight: 600`，`font-size: 3rem`
- **无英文展示字体**: 页面标题和按钮均为中文，不涉及英文展示字体选择
- **与站点一致性**: 这是 21 个页面中唯一一个中文标题也不使用衬线字体的页面（login/task-monitor 虽非衬线但至少是中文 UI 页）
...
- **容器宽度**: `900px`，与参考/指南页一致
- **布局结构**: 单列居中，无侧边栏或粘性导航
- **呼吸感**: `padding: 60px 20px`，头部 `margin-bottom: 60px`，符合主站慷慨间距的特点
- **网格系统**: 三列自适应卡片网格（`minmax(250px, 1fr)`），gap 为 `24px`
...
.intro {
  ...
  border-radius: 16px;
  padding: 40px;
  box-shadow: 0 4px 12px rgba(107, 144, 128, 0.08);
  margin-bottom: 40px;
}
```
- 纯白背景 + `16px` 圆角，与主站内容页常用的 `8px` 圆角偏离
- 阴影带有鼠尾草绿色调（`rgba(107, 144, 128, 0.08)`），而非标准黑色弥散阴影
...
```
- 保留了主站标志性的**左侧边框强调**（`4px solid`）和 **hover 上浮**（`translateY(-4px)`）
- 但圆角 `12px` 和阴影的鼠尾草绿染色偏离了主站卡片规范
- 卡片标题使用 emoji（📖 🎨 ✍️），这是其他 20 个页面中极少见的做法
...
- **pill 圆角**（`30px`）在整个 21 个页面中独树一帜，严重偏离站点以矩形/小圆角为主的按钮风格
- 主按钮和描边按钮均使用鼠尾草绿，而非铜橙/金色
- hover 时 `translateY(-2px)`，比其他页面的 `-4px` 至 `-8px` 更加克制
...
- 居中对齐的极简页脚
- **缺少印章/徽章**：这是主站多个页面（书法史、书画鉴赏、宇宙哲学等）最具品牌识别度的元素，landing.html 完全缺失

### Mood 评估

`landing.html` 营造的氛围更接近**现代 SaaS 产品的友好着陆页**，而非 WhiteNote 主打的"数字书房"：

- ✅ **干净、清爽、低饱和度** — 视觉疲劳小
- ❌ **缺乏暖调纸感和书卷气** — 冷灰背景 + 纯白卡片 + 系统字体
- ❌ **没有仪式感** — 无 gate、无卷轴展开、无章节化结构
- ❌ **品牌色缺失** — 看不到铜橙、古金、朱砂、墨绿的任何痕迹
- ❌ **功能页孤岛效应** — 与 login.html、task-monitor.html 形成统一的鼠尾草绿"功能页集群"

### 与站点设计语言的偏离总结

| 维度 | landing.html | 站点标准 | 偏离程度 |
|------|--------------|----------|----------|
| 背景色 | `#f7f8f9` 冷灰 | `#F9F6F0` / `#faf8f3` 暖调 | 高 |
| 中文标题字体 | 系统 sans-serif | Noto Serif SC | 极高 |
| 强调色 | `#6b9080` 鼠尾草绿 | `#e07b39` / `#C9A961` 铜橙/金 | 高 |
| 卡片圆角 | `12px` / `16px` | `8px` 为主 | 中 |
| 按钮圆角 | `30px` pill | `0px`–`4px` | 极高 |
| 页脚印章 | 无 | 80px 朱砂圆章 | 高 |
| 深色区域 | 无 | 墨绿 `#0d1f1c` | 中 |
| 左侧边框强调 | `4px #6b9080` | `3px`–`5px` accent | 低 |
| hover 上浮 | `-4px` | `-4px` 至 `-8px` | 低 |

### 设计建议（针对 landing.html）

1. **优先注入品牌色**：将 `#6b9080` 全部替换为 `#e07b39` 或 `#C9A961`，按钮、卡片边框、标题、hover 状态统一调整
2. **引入 Noto Serif SC**：`h1` 和 `.subtitle` 必须使用衬线字体，这是 WhiteNote 最核心的识别特征
3. **调整背景为暖调纸感**：`#f7f8f9` → `#F9F6F0` 或 `#faf8f3`
4. **收敛圆角**：intro `16px` → `12px` 或 `8px`；feature-card `12px` → `8px`；按钮 `30px` → `4px` 或 `8px`
5. **添加页脚印章**：在 footer 中增加一个 `.wn-seal` 组件，强化品牌记忆
6. **考虑深色 Hero 区域**：在 header 上方增加一小段墨绿背景的品牌宣言区，与 landing-v2/index-v3 的深色头图风格呼应

---

*报告生成时间：2026-04-13*
*分析范围：/root/.openclaw/workspace/whitenote/docs/public/ 下的 21 个 HTML 文件*